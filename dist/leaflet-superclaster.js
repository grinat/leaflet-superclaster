...
var external_commonjs_leaflet_commonjs2_leaflet_amd_leaflet_root_L_ = __webpack_require__(0);

// EXTERNAL MODULE: ./src/Supercluster.worker.js
var Supercluster_worker = __webpack_require__(1);
var Supercluster_worker_default = /*#__PURE__*/__webpack_require__.n(Supercluster_worker);

// CONCATENATED MODULE: ./src/WorkerMessageManager.js
...
var WorkerMessageManager =
/*#__PURE__*/
function () {
  function WorkerMessageManager() {
    var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
        _ref$onEnd = _ref.onEnd,
        onEnd = _ref$onEnd === void 0 ? function () {
      return '';
    } : _ref$onEnd,
        _ref$onWait = _ref.onWait,
        onWait = _ref$onWait === void 0 ? function () {
      return '';
    } : _ref$onWait;

    _classCallCheck(this, WorkerMessageManager);
  ...
  _createClass(WorkerMessageManager, [{
    key: "send",
    value: function send(message, cb) {
      var action = "".concat(message.action, ".").concat(message.data.clusterId || '');
    ...
    value: function receive(message) {
      var action = "".concat(message.action, ".").concat(message.clusterId || '');
      ...
      if (this._toSend[action].length > 0) {
        ...
        // others - drop
        var lastCb = this._toSend[action].pop();
    ...
    value: function checkQueue() {
      ...
      for (var action in this._toSend) {
        if (this._toSend[action].length > 0) {
          isEmpty = false;
          break;
        }
  ...
  return WorkerMessageManager;
}();
// EXTERNAL MODULE: ./src/supercluster.scss
var supercluster = __webpack_require__(4);

// CONCATENATED MODULE: ./src/SuperclusterGroup.js
...
var SuperclusterGroup = external_commonjs_leaflet_commonjs2_leaflet_amd_leaflet_root_L_["SuperclusterGroup"] = external_commonjs_leaflet_commonjs2_leaflet_amd_leaflet_root_L_["FeatureGroup"].extend({
  options: {
    ...
    supercluster: {
      ...
      log: false,
      map: null,
      // you can pass function as string `return {sum: props.myValue}`
  ...
  _initWorker: function _initWorker() {
    ...
    this._workerMessageManager = new WorkerMessageManager({
      onEnd: function onEnd() {
        return _this.fire('end');
      },
      onWait: function onWait() {
        return _this.fire('wait');
      }
    });
    this._worker = new Supercluster_worker_default.a();
  ...
  _onWorkerMessage: function _onWorkerMessage(_ref) {
    var data = _ref.data;
  ...
  _sendMessage: function _sendMessage(action) {
    ...
    this._workerMessageManager.send(message, function () {
      _this2._worker.postMessage(message);
    });
  ...
  _createGeoJsonLayer: function _createGeoJsonLayer() {
    ...
    this._geoJsonLayer = external_commonjs_leaflet_commonjs2_leaflet_amd_leaflet_root_L_["geoJson"](null, {
      pointToLayer: this._pointToLayer.bind(this)
    });

    this._geoJsonLayer.on('click', this._geoJsonClick, this);

    this._geoJsonLayer.on('layerremove', function (_ref2) {
      var layer = _ref2.layer;

      _this3._checkAndUnKeepPoint(layer);
    });

    this._geoJsonLayer.on('popupclose', function (_ref3) {
      var layer = _ref3.layer;

      _this3._checkAndUnKeepPoint(layer);
    });

    this._geoJsonLayer.on('popupopen', function (_ref4) {
      var layer = _ref4.layer;

      _this3._checkAndKeepPoint(layer);
    });

    if (this.options.animated === true) {
      this._geoJsonLayer.on('layeradd', function (_ref5) {
        var layer = _ref5.layer;

        _this3._addClassToIcon(layer, 'animate-add');
      });
    }
  },
  ...
  _geoJsonClick: function _geoJsonClick(_ref6) {
    var latlng = _ref6.latlng,
        layer = _ref6.layer;
  ...
  _checkAndKeepPoint: function _checkAndKeepPoint(layer) {
    if (layer instanceof external_commonjs_leaflet_commonjs2_leaflet_amd_leaflet_root_L_["Marker"] && !layer.feature.properties.subCluster) {
  ...
  moveToLastKept: function moveToLastKept() {
    ...
    var bounds = external_commonjs_leaflet_commonjs2_leaflet_amd_leaflet_root_L_["latLngBounds"](layer.getLatLng().toBounds(distMin * this.options.moveToLastKeptBoundsMultiplier));
  ...
  _drawItems: function _drawItems(features, zoom) {
    ...
    if (this.options.optimizeRedraw === false || len === 0) {
      ...
      layers.forEach(function (l) {
        _this4._deleteLayerFromGeoJsonLayer(l);
      });
  ...
  _removeOrUpdateLayer: function _removeOrUpdateLayer(l, featureIdMap, propKey) {
    ...
    if (featureIdMap[id]) {
      // update marker pos
      l.setLatLng(new external_commonjs_leaflet_commonjs2_leaflet_amd_leaflet_root_L_["LatLng"](featureIdMap[id].geometry.coordinates[1], featureIdMap[id].geometry.coordinates[0]));
  ...
  _pointToLayer: function _pointToLayer(feature, latlng) {
    var isCluster = feature.properties.subCluster || feature.properties.cluster;

    if (isCluster) {
      return external_commonjs_leaflet_commonjs2_leaflet_amd_leaflet_root_L_["marker"](latlng, {
  ...
  _clusterIconFunc: function _clusterIconFunc(feature) {
    return new external_commonjs_leaflet_commonjs2_leaflet_amd_leaflet_root_L_["DivIcon"]({
  ...
  _createMarkersInCluster: function _createMarkersInCluster(parentLayer, features, legs) {
    ...
    parentLayer._openedClusterLayer = external_commonjs_leaflet_commonjs2_leaflet_amd_leaflet_root_L_["geoJson"]([].concat(_toConsumableArray(features), _toConsumableArray(legs)), {
      pointToLayer: this._pointToLayer.bind(this),
  ...
  _recursiveRemoveAllOpenedClusterLayer: function _recursiveRemoveAllOpenedClusterLayer(l) {
    ...
    if (l._openedClusterLayer) {
      ...
      layers.forEach(function (subL) {
        _this6._recursiveRemoveAllOpenedClusterLayer(subL);
  ...
  _getDistanceFromMapCenter: function _getDistanceFromMapCenter() {
    ...
    var centerEast = external_commonjs_leaflet_commonjs2_leaflet_amd_leaflet_root_L_["latLng"](center.lat, bounds.getEast());
    var distCenterToEast = center.distanceTo(centerEast);
    var centerNorth = external_commonjs_leaflet_commonjs2_leaflet_amd_leaflet_root_L_["latLng"](bounds.getNorth(), center.lng);
  ...
  _createSpiral: function _createSpiral(features, parentCenter) {
    ...
    for (i = count; i >= 0; i--) {
      ...
      if (i < count) {
        p = new external_commonjs_leaflet_commonjs2_leaflet_amd_leaflet_root_L_["Point"](centerPt.x + legLength * Math.cos(angle), centerPt.y + legLength * Math.sin(angle))._round();

...
function supercluster_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function supercluster_defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } }

function supercluster_createClass(Constructor, protoProps, staticProps) { if (protoProps) supercluster_defineProperties(Constructor.prototype, protoProps); if (staticProps) supercluster_defineProperties(Constructor, staticProps); return Constructor; }
...
var supercluster_Supercluster =
/*#__PURE__*/
function () {
  function Supercluster(options) {
    supercluster_classCallCheck(this, Supercluster);

    this.options = extend(Object.create(defaultOptions), options);
    this.trees = new Array(this.options.maxZoom + 1);
  }

  supercluster_createClass(Supercluster, [{
    key: "load",
    ...
    value: function _cluster(points, zoom) {
      ...
      for (var i = 0; i < points.length; i++) {
        ...
        var clusterProperties = reduce ? this._map(p, true) : null; // encode both zoom and point index on which the cluster originated
...
// CONCATENATED MODULE: ./node_modules/babel-loader/lib!./src/Supercluster.worker.js
function Supercluster_worker_slicedToArray(arr, i) { return Supercluster_worker_arrayWithHoles(arr) || Supercluster_worker_iterableToArrayLimit(arr, i) || Supercluster_worker_nonIterableRest(); }

function Supercluster_worker_nonIterableRest() { throw new TypeError("Invalid attempt to destructure non-iterable instance"); }

function Supercluster_worker_iterableToArrayLimit(arr, i) { var _arr = []; var _n = true; var _d = false; var _e = undefined; try { for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"] != null) _i["return"](); } finally { if (_d) throw _e; } } return _arr; }

function Supercluster_worker_arrayWithHoles(arr) { if (Array.isArray(arr)) return arr; }



var cluster = null;
var lastLoadedFeatures = null;
var childPointsIdsMap = {};
var clusterHashMap = {};
...
function pointsInCluster(_ref2) {
  var clusterId = _ref2.clusterId;
  var features = cluster.getLeaves(clusterId, Infinity);
...
function expansionZoom(_ref3) {
  var clusterId = _ref3.clusterId,
      latlng = _ref3.latlng;
  var zoom = cluster.getClusterExpansionZoom(clusterId);
...
function clusteringData(_ref4, _ref5) {
  var _ref4$keptPointIds = _ref4.keptPointIds,
      keptPointIds = _ref4$keptPointIds === void 0 ? [] : _ref4$keptPointIds,
      bbox = _ref4.bbox,
      zoom = _ref4.zoom;
  var log = _ref5.log,
      bboxIncreasePer = _ref5.bboxIncreasePer,
      appendChildIdsToCluster = _ref5.appendChildIdsToCluster,
      optimizeRedraw = _ref5.optimizeRedraw;
  ...
  var _bbox = Supercluster_worker_slicedToArray(bbox, 4),
      w = _bbox[0],
      s = _bbox[1],
      e = _bbox[2],
      n = _bbox[3];

  var ewLen = (e - w) * bboxIncreasePer;
  if (ewLen < 0) ewLen = ewLen * -1;
  var nsLen = (n - s) * bboxIncreasePer;
  if (nsLen < 0) nsLen = nsLen * -1;
  var increasedBbox = [w - ewLen, s - nsLen, e + ewLen, n + nsLen];
  var features = cluster.getClusters(increasedBbox, zoom);
  var hasKeptPoints = keptPointIds.length > 0;
  var grabChild = appendChildIdsToCluster || optimizeRedraw;
  ...
  var _loop = function _loop(i) {
    ...
    if (features[i].properties.cluster && grabChild === true) {
      ...
      var childIds = childPointsIdsMap[features[i].properties.cluster_id];
      ...
      if (optimizeRedraw === true) {
        if (!clusterHashMap[features[i].properties.cluster_id]) {
          clusterHashMap[features[i].properties.cluster_id] = getHashOfString(childIds.sort().join(';')).toString();
        }
      ...
      if (hasKeptPoints === true) {
        // TODO to for
        keptPointIds.forEach(function (id) {
  ...
  if (grabChild) {
    ...
    keptPointIds.forEach(function (id) {
      if (ids.indexOf(id) === -1 && lastLoadedFeatures) {
        ...
        var len = lastLoadedFeatures.length;
...
function getChildPointsIds(clusterId) {
  ...
  while (stack.length) {
    var id = stack.pop();
    var childs = cluster.getChildren(id);
...
function loadFeatures(_ref6, _ref7) {
  var _ref6$features = _ref6.features,
      features = _ref6$features === void 0 ? [] : _ref6$features;
  var supercluster = _ref7.supercluster;
